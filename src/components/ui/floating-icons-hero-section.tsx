...
const IconApple = (props: React.SVGProps<SVGSVGElement>) => (
    <svg {...props} viewBox="0 0 24 24" fill="currentColor" className="text-foreground/80 text-[#071E49]" xmlns="http://www.w3.org/2000/svg">
...
const IconVercel = (props: React.SVGProps<SVGSVGElement>) => (
    <svg {...props} viewBox="0 0 24 24" fill="currentColor" className="text-foreground/90 text-[#071E49]" xmlns="http://www.w3.org/2000/svg">
...
const Icon = ({
  ...
  return (
    ...
    >
      ...
      <motion.div
        className="flex items-center justify-center w-16 h-16 md:w-20 md:h-20 p-3 rounded-3xl shadow-xl bg-white/80 backdrop-blur-md border border-gray-200"
        animate={{
          y: [0, -8, 0, 8, 0],
          x: [0, 6, 0, -6, 0],
          rotate: [0, 5, 0, -5, 0],
...
>(({ className, title, highlightedTitle, subtitle, ctaText, ctaHref, secondaryCtaText, secondaryCtaHref, bullets, ...props }, ref) => {
  ...
  return (
    <section
      ...
      className={cn(
        'relative w-full h-screen min-h-[700px] flex items-center justify-center overflow-hidden bg-gradient-to-b from-blue-50 to-white pt-24',
        className
      )}
      {...props}
    >
      ...
      <div className="relative z-10 text-center px-4 max-w-4xl mx-auto">
        <h1 className="text-4xl lg:text-5xl xl:text-6xl font-bold tracking-tight text-[#071E49] mb-6">
          {title} <span className="text-[#1266F1]">{highlightedTitle}</span>
        </h1>
        <p className="mt-6 max-w-2xl mx-auto text-lg text-gray-600 leading-relaxed">
          {subtitle}
        </p>
        <div className="mt-10 flex flex-col sm:flex-row items-center justify-center gap-4">
          <Button asChild size="lg" className="px-8 py-6 text-base font-semibold rounded-full bg-[#1266F1] hover:bg-blue-600 shadow-sm transition-all">
            <a href={ctaHref}>{ctaText}</a>
          </Button>
          {secondaryCtaText && secondaryCtaHref && (
            <Button asChild variant="outline" size="lg" className="px-8 py-6 text-base font-semibold rounded-full shadow-sm ring-1 ring-gray-200 hover:bg-gray-50 transition-all text-[#071E49] bg-white">
        ...
        {bullets && bullets.length > 0 && (
          <div className="mt-12 flex flex-wrap justify-center gap-x-6 gap-y-3 text-sm font-medium text-gray-600">
            {bullets.map((bullet, idx) => (
              <div key={idx} className="flex items-center gap-2 bg-white/60 backdrop-blur-sm px-4 py-2 rounded-full shadow-sm border border-gray-100">
                <CheckCircle2 className="h-4 w-4 text-[#1266F1]" /> {bullet}
              </div>
            ))}
          </div>
        )}
